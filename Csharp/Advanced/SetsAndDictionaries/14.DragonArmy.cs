...
namespace Csharp.Advanced.SetsAndDictionaries
{
    ...
    {
        public static class _14DragonArmy
        {
            public static void Solution()
            {
                ...
                var dragonInfo = new Dictionary<string, SortedDictionary<string, Dictionary<string, int>>>();
                ...
                while (t-- > 0)
                {
                    ...
                    {
                        dragonInfo[type]
                            .Add(name, new Dictionary<string, int>
                            {
                                { "damage", damage },
                                { "health", health },
                                { "armor", armor }
                ...
                {
                    // magic
                    var avgDmg = dragonType.Value
                        .Select(x => x.Value.Where(y => y.Key == "damage"))
                        .Select(x => x.Select(y => y.Value).Average())
                        .Average();

                    var avgHealth = dragonType.Value
                        .Select(x => x.Value.Where(y => y.Key == "health"))
                        .Select(x => x.Select(y => y.Value).Average())
                        .Average();

                    var avgArmor = dragonType.Value
                        .Select(x => x.Value.Where(y => y.Key == "armor"))
                        .Select(x => x.Select(y => y.Value).Average())
                        .Average();
