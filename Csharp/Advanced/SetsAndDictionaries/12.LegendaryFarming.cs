...
namespace Csharp.Advanced.SetsAndDictionaries
{
    ...
    {
        public static class _12LegendaryFarming
        {
            static void Solution()
            {
                // Dictionary {string TYPE, {SortedDictionary { string MATERIAL, int AMOUNT }}}
                var inventory = new Dictionary<string, SortedDictionary<string, int>>();

                var specialItems = new Dictionary<string, string>
                {
                    { "Shadowmourne", "shards" },
                    { "Valanyr", "fragments" },
                    { "Dragonwrath","motes" }
                ...
                while (true)
                {
                    ...
                    for (int i = 0; i < items.Length; i++)
                    {
                        ...
                        if (specialItems.ContainsValue(items[i]))
                        {
                            ...
                            if (inventory["legendary"][items[i]] < 250)
                                continue;
